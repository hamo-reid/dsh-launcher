 ...
 * profile is limited to a single live process (`core/shared/run-registry.ts` enforces
...
import { existsExecutable, resolveLaunchEntry, type LaunchEntry } from '../../core/dsh/dsh.ts'
import { buildDshLaunch } from '../../core/profile/launch-spec.ts'
import { findInsertConflicts } from '../../core/store/combo.ts'
...
import { contextForEntry, dshEntryById } from '../../core/profile/appState.ts'
import {
  profileId, readLaunchOptions, readRunMode, writeLaunchOptions, writeRunMode,
} from '../../core/profile/launch-config.ts'
import { fail, failFromError, E } from '../../core/shared/errors.ts'
import { child, logger } from '../../core/shared/logger.ts'
import { effectiveArgs, sanitizeLaunchOptions } from '../../core/profile/launch-options.ts'
import { hasRun, nextRunId } from '../../core/shared/run-registry.ts'
import { killProcessTree } from '../../core/shared/process-kill.ts'
 ...
 * Detection + the decision live in `core/dsh/node-env.ts` (shared with the settings
...
export function registerRunIpc(): void {
  handle('run:start', (_event, profile: string, mode?: RunMode, options?: LaunchOptions, dshId?: string): IpcResult<{ id: string }> => {
    ...
    try {
      ...
      const { cwd } = launch
      // argv/env are assembled in one place (core/profile/launch-spec.ts) so app and shell
