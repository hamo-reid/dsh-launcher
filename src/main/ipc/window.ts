...
export function registerWindowIpc(): void {
  handle('window:minimize', (event): IpcResult<boolean> => {
  ...
  handle('window:toggleMaximize', (event): IpcResult<boolean> => {
    const w = windowFrom(event)
    if (w === undefined) return { ok: false, code: 'window.notFound', error: 'no window' }
  ...
  handle('window:close', (event): IpcResult<boolean> => {
    const w = windowFrom(event)
    if (w === undefined) return { ok: false, code: 'window.notFound', error: 'no window' }
  ...
  handle('window:quit', (): IpcResult<boolean> => {
  ...
  handle('window:isMaximized', (event): IpcResult<boolean> => {
    const w = windowFrom(event)
    if (w === undefined) return { ok: false, code: 'window.notFound', error: 'no window' }
