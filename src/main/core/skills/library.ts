 ...
 * `core/skills/skills.ts` — a skill the launcher accepts here is one dsh loads.
 * Deletion moves the entry to the OS recycle bin through an injected mover.
 */
import {
  cpSync, existsSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, renameSync, rmSync, writeFileSync,
} from 'node:fs'
