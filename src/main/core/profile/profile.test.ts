...
vi.mock('../store/plugins.ts', async (importActual) => {
  const actual = await importActual<typeof import('../store/plugins.ts')>()
  return {
    ...actual,
...
import { addLocalPlugin, addPlugin, installIntoProfile } from '../store/plugins.ts'
