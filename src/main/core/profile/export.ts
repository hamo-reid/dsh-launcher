...
import { PATCH_FILE_NAME } from '../patch/patch.ts'
import { PNPM_WORKSPACE_YAML, runPnpm, type PnpmResult } from '../dsh/pnpm.ts'
import { addLocalPlugin, addPlugin, installIntoProfile, installedStoreVersion } from '../store/plugins.ts'
import { versionsRoot } from '../store/layout.ts'
import { pathOutsideRoot } from './name-guard.ts'
import { satisfiesRange } from '../shared/version.ts'
import { writeNewProfileId } from './launch-config.ts'
import { assertCustomProfileName } from './lifecycle.ts'
import type {
