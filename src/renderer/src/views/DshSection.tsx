 ...
 *
 * 这里**没有全局「当前 DSH」**：列表选中只决定本页右侧查看/编辑哪个安装，
 * 不会改变其它页面（Profile / 运行页各自选择自己的 dsh）。 */
export default function DshSection() {
  const { t } = useTranslation()
  const { token } = theme.useToken()
  const [dshes, setDshes] = useState<DshEntry[]>([])
  const [loading, setLoading] = useState(true)
  // Local view selection only — not a persisted "active dsh".
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    // Keep a still-valid selection, else fall back to the first install.
    setSelectedId(prev => (prev !== undefined && r.value.dshes.some(d => d.id === prev)) ? prev : r.value.dshes[0]?.id)
    setLoading(false)
  ...
  // The home editor follows the local selection.
  useEffect(() => {
    setHomeInput(selected?.home ?? '')
  }, [selectedId, dshes])

  const saveHome = async (value: string): Promise<string> => {
    if (selectedId === undefined) return t('dsh.noDshSelected')
    const r = await window.api.dsh.setHome(selectedId, value)
  ...
  const exportData = async (): Promise<void> => {
    ...
    const r = await window.api.data.export(selectedId)
    if (!r.ok) { void message.error(apiErrorText(r)); return }
    if (r.value === '') return
  ...
  const importData = async (): Promise<void> => {
    if (selectedId === undefined) return
    const entry = dshes.find(d => d.id === selectedId)
  ...
  return (
      ...
      {selected !== undefined
        ? (
          <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
            <SectionHeading title={selected.name} />
            <Panel title={t('dsh.basicInfo')}>
              <Descriptions size="small" column={1} bordered>
                <Descriptions.Item label={t('dsh.version')}>{selected.version || t('common.unknown')}</Descriptions.Item>
                <Descriptions.Item label={t('dsh.launchCommand')}>
                  <Typography.Text copyable code style={{ wordBreak: 'break-all' }}>
                    {selected.launch ?? selected.execPath}
                  </Typography.Text>
                </Descriptions.Item>
                <Descriptions.Item label={t('dsh.location')}>
                  <Space size="small">
                    <Typography.Text copyable code style={{ wordBreak: 'break-all' }}>
                      {selected.dir ?? selected.execPath}
                    </Typography.Text>
                    <Button size="small" onClick={() => void revealDir(selected.id)}>{t('dsh.reveal')}</Button>
                  </Space>
                </Descriptions.Item>
              </Descriptions>
            </Panel>
            <Panel title={t('dsh.dirConfig')}>
              ...
              <div style={{ color: token.colorTextTertiary, fontSize: token.fontSizeSM }}>
                {t('dsh.effectiveDir', { dir: selected.profileDir ?? '—' })}
              </div>
              {selected.legacyProfilesDir !== undefined && (
                <Alert
                  type="warning"
                  showIcon
                  style={{ marginTop: 10 }}
                  title={t('dsh.legacyProfilesDirTitle')}
                  description={
                    <div>
                      <div>{t('dsh.legacyProfilesDirDesc', { dir: selected.legacyProfilesDir })}</div>
                      <Typography.Text copyable code style={{ wordBreak: 'break-all', marginTop: 6, display: 'inline-block' }}>
                        {selected.legacyProfilesDir}
                      </Typography.Text>
                    </div>
                  }
                />
    ...
    <DataMirrorModal
      open={mirrorOpen}
      source={selected === undefined ? null : { id: selected.id, name: selected.name, version: selected.version }}
