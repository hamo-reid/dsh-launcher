...
export default function DevPluginsView(): JSX.Element {
  ...
  const diagnose = async (name: string): Promise<void> => {
    setBusy(`diag:${name}`)
    const r = await window.api.plugins.devDiagnose(name)
    setBusy('')
  ...
  const shim = async (name: string): Promise<void> => {
    setBusy(`shim:${name}`)
    const r = await window.api.plugins.devShimPeers(name)
    setBusy('')
    if (!r.ok) { void message.error(apiErrorText(r)); return }
        ...
        {detail !== null && (
          <Space orientation="vertical" size="small" style={{ width: '100%' }}>
            ...
            <div>
              ...
              {detail.diag.patchRows.length === 0
                ...
                : detail.diag.patchRows.map(r => (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0' }}>
                    <Tag style={{ fontFamily: 'monospace' }}>{r.id}</Tag>
                    <span style={{ fontFamily: 'monospace', fontSize: token.fontSizeSM }}>{r.name === '' ? '-' : r.name}</span>
                    {r.name === '' ? null : r.dir !== undefined
                      ? <Tag color={r.root === 'monorepo' ? 'geekblue' : 'success'}>{rootLabel(r.root)}</Tag>
                      : <Tag color="error">{t('plugin.dev.diagMissing')}</Tag>}
                  </div>
                ))}
            </div>
            <div>
              ...
              {detail.diag.peers.length === 0
                ...
                : detail.diag.peers.map(peer => (
                  <div key={peer.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: token.fontSizeSM }}>{peer.name}</span>
                    {peer.dir !== undefined
                      ? <Tag color={peer.root === 'monorepo' ? 'geekblue' : 'orange'}>{rootLabel(peer.root)}</Tag>
                      : <Tag color="error">{t('plugin.dev.diagMissing')}</Tag>}
            ...
            <Space wrap>
              ...
              <Button loading={busy === `shim:${detail.name}`} onClick={() => void shim(detail.name)}>{t('plugin.dev.fixShim')}</Button>
